...
import wandb
from src.models.pgfs.models.actor import ActorNetwork, FNetwork, PiNetwork
...
class TD3:
    def __init__(
        ...
        self.actor_procedure = ActorProcedure(state_dim, template_dim, action_dim).to(
            self.device
        )
        self.actor_procedure_target = ActorProcedure(
            state_dim, template_dim, action_dim
        ).to(self.device)
        self.actor_procedure_target.load_state_dict(self.actor_procedure.state_dict())

        self.critic1 = CriticNetwork(state_dim, template_dim, action_dim).to(
            self.device
        )
        self.critic2 = CriticNetwork(state_dim, template_dim, action_dim).to(
            self.device
        )
        self.critic1_target = CriticNetwork(state_dim, template_dim, action_dim).to(
            self.device
        )
        self.critic2_target = CriticNetwork(state_dim, template_dim, action_dim).to(
            self.device
        )
    ...
    def backward(self, replay_buffer, batch_size):
        ...
        state, state_tmask, template, action, next_state, next_tmask, reward, done = (
        ...
        with torch.no_grad():
            next_template, next_action = self.actor_procedure_target(
                next_state, self.temperature, next_tmask
            )
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # Compute actor loss
            template, action = self.actor_procedure(
                state, self.temperature, state_tmask
            )
            ...
            self.f_net_loss = nn.CrossEntropyLoss()(
                self.actor_procedure.f_net(state), target_templates
            )
        ...
        self.temperature = max(
            self.min_temp, self.temperature * self.temp_decay
        )  # Decay temperature with a factor (e.g., 0.99)
    ...
    def save(self, filename, replay_buffer):
        ...
        checkpoint = {
            ...
            "actor_loss": (
                self.actor_loss.item() if self.actor_loss is not None else None
            ),
            "critic1_loss": (
                self.critic1_loss.item() if self.critic1_loss is not None else None
            ),
            "critic2_loss": (
                self.critic2_loss.item() if self.critic2_loss is not None else None
            ),
    ...
    def load(self, filename, replay_buffer):
        ...
        self.actor_procedure_target.load_state_dict(
