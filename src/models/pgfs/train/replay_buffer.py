...
class ReplayBuffer:
    ...
    def add(self, state, template, action, next_state, reward, done):
        """
    ...
    def sample(self, batch_size):
        ...
        state, template, action, next_state, reward, done = map(torch.stack, zip(*batch))
        return state.squeeze(1), template.squeeze(1), action.squeeze(1), next_state.squeeze(1), reward.unsqueeze(1), done.unsqueeze(1)
