...
import wandb
from src.models.pgfs.envs.environment import Environment
...
def main(experiment_name):
    ...
    env = Environment(
        precomputed_vectors_file=molecule_file,
        templates_file=template_file,
        max_steps=max_steps,
    )
    ...
    if checkpoint_files:
        ...
        logger.info(
            f"Checkpoints found. Resuming training using checkpoint: {latest_checkpoint} with time step: {current_time_step}..."
        )
    else:
    ...
    while current_time_step < total_time_steps:
        state, state_uid, state_tmask = (
            env.reset()
        )  # Reset the environment and get initial state and its UUID
        ...
        for step in range(max_steps):
            ...
            next_state, next_state_uid, reward, done, next_tmask = env.step(
                state_uid, template, action
            )
        ...
        if replay_buffer.size() > batch_size:
            ...
            logger.info(
                f"Episode {episode} - Current time steps: {current_time_step} Critic1 Loss: {td3.critic1_loss}, Critic2 Loss: {td3.critic2_loss}, Actor Loss: {td3.actor_loss}, Fnet Loss {td3.f_net_loss}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiment_name", type=str, required=True, help="Custom experiment name"
    )
