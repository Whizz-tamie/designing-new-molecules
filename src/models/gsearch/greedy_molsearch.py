...
class GreedyMolSearch:
    ...
    def generate_new_molecule(self, current_reactant, templates, step, path):
        ...
        for template in templates:
            ...
            if rxn.GetNumReactantTemplates() == 1:
                ...
                possible_products = self.forward_reaction(
                    current_reactant, template.Smarts
                )
            elif rxn.GetNumReactantTemplates() == 2:
                compatible_reactants = [
                    r
                    for r in self.reactants
                    if Chem.MolFromSmiles(r).HasSubstructMatch(
                        rxn.GetReactantTemplate(1)
                    )
                ]
                logging.info(
                    f"Found {len(compatible_reactants)} compatible second reactants for bimolecular template {template.Reaction}..."
                )
                if not compatible_reactants:
                    continue

                # Select the second reactant that maximizes the clogP score of the resulting product
                for reactant2 in compatible_reactants:
                    self.total_reactions += 1  # Increment for each attempt
                    possible_products = self.forward_reaction(
                        current_reactant, template.Smarts, reactant2=reactant2
                    )
                    if possible_products:
                        clogp_scores = [
                            self.compute_clogp(product) for product in possible_products
                        ]
                        max_clogp = max(clogp_scores)
                        if max_clogp > best_clogp:
                            best_clogp = max_clogp
                            best_template = template
                            best_second_reactant = reactant2
                            best_product = possible_products[
                                clogp_scores.index(max_clogp)
                            ]
            else:
                continue
            ...
            if possible_products is not None and best_template is None:
                ...
                if max_clogp > best_clogp:
                    best_clogp = max_clogp
                    best_template = template
                    best_product = possible_products[clogp_scores.index(max_clogp)]

        if best_product:
            logging.info(
                f"Selected best template:{best_template.Reaction} and best second reactant:{best_second_reactant} producing product:{best_product} with highest clogP:{best_clogp}..."
            )
            step_info = SynthesisStep(
                step=step,
                reactant=current_reactant,
                template=best_template.Reaction,
                product=best_product,
                clogp=best_clogp,
                second_reactant=best_second_reactant,
            )
            ...
            logging.info(
                f"No valid products generated for {current_reactant} using any template."
            )
    ...
    def run_molsearch(self):
        try:
            logging.info(
                f"Starting Greedy molecule search... Date:{datetime.now().strftime('%Y%m%d_%H%M%S')}..."
            )
            for idx, initial_reactant in enumerate(self.reactants):
                ...
                logging.info(
                    f"Path:{self.path_id_counter}: Selected starting reactant:{initial_reactant}"
                )
                ...
                for step in range(1, self.max_steps + 1):
                    ...
                    logging.info(
                        f"Found {len(compatible_templates)} compatible templates for reactant {current_reactant}..."
                    )
                    ...
                    current_reactant, valid_step = self.generate_new_molecule(
                        current_reactant, compatible_templates, step, path
                    )
                    if not valid_step:
                        logging.info(
                            f"Path:{self.path_id_counter} ends in Step:{step}: No valid products generated."
                        )
                        break
