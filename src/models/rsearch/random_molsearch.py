...
class RandomMolSearch:
    def __init__(self, reactants, templates, experiment_name, checkpoint_file, result_file, max_steps=5, max_reactions=100, max_attempts=100):
        ...
        wandb.init(project="random_molsearch",
                entity="whizz",
    ...
    def generate_new_molecule(self, current_reactant, template, step, path):
        ...
        if rxn.GetNumReactantTemplates() == 1:
            ...
            logging.info(f"Found {len(compatible_reactants)} valid secod reactants for {current_reactant} and randomly select {second_reactant}")
            products = self.forward_reaction(current_reactant, template.Smarts, reactant2=second_reactant)
        ...
        if products:
            ...
            logging.info(f"No valid products generated for {current_reactant} using reaction template {template.Reaction}")
            return current_reactant, False
    ...
    def run_molsearch(self):
        try:
            logging.info(f"Starting Random molecule search... Date:{datetime.now().strftime('%Y%m%d_%H%M%S')}...")
            while self.total_reactions < self.max_reactions and self.attempts < self.max_attempts:
                ...
                logging.info(f"Path:{self.path_id_counter}: Randomly selected starting reactant:{initial_reactant}")
                ...
                for step in range(1, self.max_steps + 1):
                    ...
                    logging.info(f"Found {len(compatible_templates)} templates for {current_reactant} and randomly selected {template.Reaction}...")
                    current_reactant, valid_step = self.generate_new_molecule(current_reactant, template, step, path)
                    if not valid_step:
                        logging.info(f"Path:{self.path_id_counter} ends in Step:{step}: No products generated by reactions...")
                        break
