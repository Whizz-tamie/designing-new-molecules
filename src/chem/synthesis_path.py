...
def draw_path(df, path_id):
    ...
    for _, row in path_df.iterrows():
        if row["step"] == 0:
            ...
            ax.axis('off')
            fig.text(col_idx / num_columns + 0.5 / num_columns, 0.35, f'QED: {row["qed"]:.3f}', ha='center', fontsize=12)
            col_idx += 1

        elif row["step"] > 0:
            ax = fig.add_axes([(col_idx - 0.1) / num_columns, 0.3, 1 / num_columns, 0.5])
            arrow = FancyArrow(0.1, 0.5, 0.8, 0, width=0.002, head_width=0.06, head_length=0.1, color='black')
            ax.add_patch(arrow)
            ax.axis('off')
            # Annotate the reaction name close to the arrow
            fig.text((col_idx - 0.1) / num_columns + 0.5 / num_columns, 0.5, row['template'], ha='center', fontsize=12)
            col_idx += 1
            ...
            if pd.notna(row['second_reactant']):
                ...
                # Plus sign
                fig.text((col_idx - 1.1) / num_columns + 0.5 / num_columns, 0.6, "+", ha='center', fontsize=20)
